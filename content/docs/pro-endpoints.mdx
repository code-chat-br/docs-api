---
title: "Recursos Pro"
description: "Classificação comercial dos recursos Pro e comportamento real do runtime."
---
...
O JWT de usuario precisa conter `userId` em formato UUID e `exp`. A expiracao sempre e validada. A classificacao e apenas documental no runtime atual e nao altera respostas nem adiciona `402`. Consulte [Envio em lote](/docs/message-batches) para arquitetura, estados, payloads, recovery e webhooks.
...
A classificacao e apenas documental no runtime atual e nao altera respostas nem adiciona `402`. No OpenAPI, cada endpoint de Calls usa `x-codechat-plan: pro` e `x-codechat-plan-enforced: false`. Consulte [Chamadas WhatsApp](/docs/calls) e [API de chamadas](/docs/calls-api) para o contrato completo.
...
Todos os interativos rejeitam `mentionAll`. Consulte [Mensagens](/docs/messages) para destinatários e opções compartilhadas.
