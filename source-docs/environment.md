...
| `MESSAGE_BATCH_WORKER_ENABLED` | Não | `true` | Habilita o worker persistente de Envio em lote. Com `false`, os endpoints e os dados continuam disponíveis, mas nenhum item é consumido. |
| `MESSAGE_BATCH_WORKER_POLL_INTERVAL_MS` | Não | `1000` | Intervalo, em milissegundos, entre consultas do worker e da outbox. Precisa ser maior que zero. |
