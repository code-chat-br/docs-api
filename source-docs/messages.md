...
Na resposta dos endpoints de envio, `messageType` pode ser `imageMessage`, `audioMessage`, `videoMessage`, `documentMessage`, `stickerMessage`, `extendedTextMessage`, `interactiveMessage`, `contactMessage`, `contactArrayMessage`, `locationMessage` ou `reactionMessage`. `content` sempre é um objeto relativo ao `messageType` retornado.
