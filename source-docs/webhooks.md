...
- [Eventos do Message Batch](#eventos-do-message-batch)
- [Eventos](#eventos)
...
```http
PUT /instance/codechat/webhook HTTP/1.1
...
```http
GET /instance/codechat/webhook HTTP/1.1
Authorization: Bearer <token-da-instancia>
```
...
| `global only` | `message.batch.waiting-instance` | Evento persistente do Envio em lote; nao possui flag por instancia. |
| `global only` | `message.batch.resumed` | Evento persistente do Envio em lote; nao possui flag por instancia. |
| `global only` | `message.batch.stop-requested` | Evento persistente do Envio em lote; nao possui flag por instancia. |
| `global only` | `message.batch.stopped` | Evento persistente do Envio em lote; nao possui flag por instancia. |
| `global only` | `message.batch.interrupted` | Evento persistente do Envio em lote; nao possui flag por instancia. |
| `global only` | `message.batch.completed` | Evento persistente do Envio em lote; nao possui flag por instancia. |
| `global only` | `message.batch.completed-with-errors` | Evento persistente do Envio em lote; nao possui flag por instancia. |
| `global only` | `message.batch.item.failed` | Evento persistente do Envio em lote; nao possui flag por instancia. |
| `global only` | `message.batch.item.unknown` | Evento persistente do Envio em lote; nao possui flag por instancia. |
...
## Eventos do Message Batch

Esses eventos sao enviados uma unica vez ao `WEBHOOK_GLOBAL_URL`; nao sao repetidos por instancia. A linha de outbox e criada no mesmo commit da alteracao do lote/item, e a chamada HTTP acontece depois da transacao. O mesmo payload tambem e publicado no WebSocket global `/ws/global/events` quando o evento esta assinado pelo `ownerUserId` do JWT de usuario.

### Mapa de eventos de lote
...
| `message.batch.waiting-instance` | Nenhuma instancia do lote esta conectada. |
| `message.batch.resumed` | Lote foi retomado ou uma instancia voltou. |
| `message.batch.stop-requested` | API aceitou a solicitacao de stop. |
| `message.batch.stopped` | Worker concluiu o stop controlado. |
| `message.batch.interrupted` | Lease expirou ou o processo encerrou. |
| `message.batch.completed` | Todos os itens terminaram sem falha ou UNKNOWN. |
| `message.batch.completed-with-errors` | Todos os itens terminaram com falha ou UNKNOWN. |
| `message.batch.item.failed` | Um item terminou em FAILED. |
| `message.batch.item.unknown` | O resultado externo de um item nao pode ser confirmado. |

### Payloads JSON por evento

Os exemplos abaixo mostram o nome exato do evento em `event` e o conteudo da mensagem do lote em `data.batch.message`. Eventos `message.batch.item.*` usam o contrato de item e nao carregam `data.batch.message`; use `batchId` para consultar o lote quando precisar da mensagem original.
...
[
  {
    "data": {
      "batch": {
        ...
        "message": {
          "payload": {
            "text": "Ola, esta e uma mensagem em lote."
          },
          "type": "text"
        },
  ...
  },
  {
    ...
    "event": "message.batch.window-started",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
    ...
    "event": "message.batch.waiting-window",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
      "batch": {
        ...
        "lastInterruptedAt": "2026-07-13T14:58:00Z",
        "lastRecoveredAt": "2026-07-13T15:00:00Z",
        "message": {
          "payload": {
            "text": "Ola, esta e uma mensagem em lote."
          },
  ...
  },
  {
    ...
    "event": "message.batch.pause-requested",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
  ...
  },
  {
    ...
    "event": "message.batch.waiting-instance",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
      "batch": {
        ...
        "progress": 35,
        "schedule": null,
        "startedAt": "2026-07-13T15:15:00Z",
        "status": "PROCESSING",
        "updatedAt": "2026-07-13T15:10:00Z"
      }
    },
    "event": "message.batch.resumed",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
    ...
    "event": "message.batch.stop-requested",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
      "batch": {
        ...
        "schedule": null,
        "status": "STOPPED",
        "stoppedAt": "2026-07-13T15:16:00Z",
        "updatedAt": "2026-07-13T15:10:00Z"
      }
    },
    "event": "message.batch.stopped",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
      "batch": {
        ...
        "interruptedAt": "2026-07-13T15:17:00Z",
        "interruptionReason": "GRACEFUL_SHUTDOWN",
        "lastInterruptedAt": "2026-07-13T15:17:00Z",
        "lastRecoveredAt": null,
        "message": {
          "payload": {
            "text": "Ola, esta e uma mensagem em lote."
    ...
    "event": "message.batch.interrupted",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
  ...
  },
  {
    ...
    "event": "message.batch.completed-with-errors",
    "timestamp": "2026-07-13T15:10:00Z"
  },
  {
    "data": {
      ...
      "itemId": "01900000-0000-7000-8000-000000000036",
      "recipient": "5531999999999"
    },
  ...
  },
  {
    "data": {
      ...
      "error": {
        ...
        "message": "A mensagem pode ter sido enviada, mas o resultado nao foi persistido."
      },
      "instance": {
        "id": 1,
        "name": "codechat-01"
    ...
    "event": "message.batch.item.unknown",
    "timestamp": "2026-07-13T15:10:00Z"
  }
]
```

### Headers do Message Batch

| Header | Valor |
| --- | --- |
| `Content-Type` | `application/json` |
| `User-Agent` | `CodeChat-MessageBatch-Webhook/1.0` |
| `x-request-id` | UUID de rastreio da tentativa HTTP |
| `x-webhook-event` | Mesmo valor de `event` no body |

Os headers de instancia (`x-instance-id`, `x-instance-name` e `x-owner-jid`) nao sao enviados nesse fluxo global.
...
`message.batch.progress` nao e criado novamente quando o percentual inteiro permanece igual.

### Body de conclusao com falhas
...
Respostas `2xx` confirmam a entrega. Falhas sao reagendadas com backoff persistente; a outbox nao chama HTTP enquanto segura uma transacao do lote.
...
- `id`: `number`, opcional, não aceita `null`. ID interno da mensagem persistida quando encontrada no banco.
- `chatJid`: `string`, obrigatório, não aceita `null`. JID da conversa.
- `senderJid`: `string`, opcional, não aceita `null`. JID do remetente quando disponivel; omitido quando ausente.
...
- `keyId`: `string`, obrigatório, não aceita `null`. ID da mensagem no protocolo WhatsApp.
