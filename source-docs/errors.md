...
Rejeições do middleware global (`400` por aliases divergentes e `401` por token ausente ou inválido) acontecem antes do handler de lote e continuam usando o envelope geral com `messages`. Falhas internas `500` também usam o envelope geral. O OpenAPI representa os dois formatos possíveis no `400` do Message Batch.
