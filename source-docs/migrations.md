...
O runner aplica somente arquivos `*.up.sql` em `internal/database/migrations`, em ordem lexicográfica, e registra os nomes aplicados na tabela `"SchemaMigration"`. Depois disso, inicializa o `sqlstore` configurado para o Whatsmeow; `sqlstore.New` executa as migrations internas no backend selecionado.
